...
func TestErrorsInAccountCreation(t *testing.T) {
	...
	t.Run("Should give erro on wrong account name", func(t *testing.T) {
		_, err := NewConnectorWithError([]string{"fixtures/invalid_account_in_deployment.json"}, "emulator", true, output.NoneLog)
		assert.Error(t, err)
	...
	t.Run("Should fail when creating local accounts in the wrong order", func(t *testing.T) {
		g := NewConnector([]string{"fixtures/wrong_account_order_emulator.json"}, "emulator", true, output.NoneLog)
