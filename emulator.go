...
func newEmulator(key *gateway.EmulatorKey, emulatorOptions ...emulator.Option) *emulator.Blockchain {
...
func (g *EmulatorGateway) EnableAutoMine() {
	g.emulator.EnableAutoMine()
}

func (g *EmulatorGateway) DisableAutoMine() {
	g.emulator.DisableAutoMine()
}

func (g *EmulatorGateway) ExecuteAndCommitBlock(t *testing.T) (*flowgo.Block, []TransactionResult, error) { //nolint:thelper
	block, results, err := g.emulator.ExecuteAndCommitBlock()
	if err != nil {
		return nil, nil, err
	}

	wrappedResults := make([]TransactionResult, len(results))
	for i, res := range results {
		formattedEvents := make([]*FormatedEvent, len(res.Events))
		for i, event := range res.Events {
			ev := ParseEvent(event, uint64(0), time.Unix(0, 0), []string{})
			formattedEvents[i] = ev
		}
		wrappedResults[i] = TransactionResult{
			Err:     err,
	...
	return block, wrappedResults, nil
}
